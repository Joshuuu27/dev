  ...
  const columns: ColumnDef<Driver>[] = [
    {
      ...
      cell: ({ row }) => {
        ...
        const noLicense = hasNoLicense(driver);
  ...
  return (
    <>
      ...
      <main className="max-w-5xl mx-auto px-6 py-8 space-y-6">
        ...
        {/* Drivers Without License Alert */}
        {drivers.filter((d) => hasNoLicense(d)).length > 0 && (
          <div className="p-4 bg-amber-50 border border-amber-300 rounded-md flex items-center gap-3">
            <AlertTriangle className="w-5 h-5 text-amber-600 flex-shrink-0" />
            <span className="text-sm font-semibold text-amber-800">
              {drivers.filter((d) => hasNoLicense(d)).length} driver(s) without a license number
            </span>
