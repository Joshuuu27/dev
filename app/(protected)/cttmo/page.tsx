      ...
      <main className="min-h-screen bg-[#F8F8FA] px-4 sm:px-6 py-8">
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
            {/* Drivers Card */}
            <Card className="border border-slate-100 bg-white rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.06)] hover:shadow-[0_8px_32px_rgba(0,0,0,0.08)] transition-shadow cursor-pointer" onClick={() => router.push("/cttmo/drivers")}>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="p-3 bg-[#EDEEF9] rounded-xl">
            ...
            <Card className="border border-slate-100 bg-white rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.06)] hover:shadow-[0_8px_32px_rgba(0,0,0,0.08)] transition-shadow cursor-pointer" onClick={handleShowDuplicates}>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`p-3 rounded-xl ${duplicateLicenseMap.duplicateLicenses.size > 0 ? 'bg-amber-50' : 'bg-slate-100'}`}>
                      <AlertTriangle className={`w-6 h-6 ${duplicateLicenseMap.duplicateLicenses.size > 0 ? 'text-amber-600' : 'text-slate-600'}`} />
                    </div>
                    <div>
            ...
            <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
              ...
              <div className="space-y-4">
                {duplicateDrivers.length > 0 ? (
                        ...
                        >
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <p className="font-semibold text-slate-900">{driver.displayName || "Unknown"}</p>
                              <div className="grid grid-cols-2 gap-4 mt-2 text-sm text-slate-600">
                                <div>
                                  <p className="text-xs text-slate-500 uppercase">License Number</p>
                  ...
                  <p className="text-center text-slate-500 py-4">No duplicate drivers found.</p>
                )}
              </div>
            </DialogContent>
          </Dialog>
