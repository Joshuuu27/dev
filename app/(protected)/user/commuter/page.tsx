  ...
  const handleToggleTracking = useCallback(() => {
    ...
    getCurrentLocationWithLabel()
      .then(({ lat, lng, label }) => {
        ...
        // Persist trip in Firestore only if user is signed in
        if (user) {
      ...
      .catch((err) => {
        ...
        if (gotLocation) {
          // Location OK – start tracking anyway; save failed
          setIsTracking(true)
          if (msg.includes("User not authenticated")) {
            alert("Tracking started. Trip could not be saved — please sign in to save trip history.")
          } else if (msg.includes("permission") || msg.includes("PERMISSION_DENIED")) {
            alert("Tracking started. Trip could not be saved. Check Firebase Console → Firestore → Rules and add rules to allow writes to the 'trips' collection for authenticated users.")
          } else {
            alert("Tracking started. Trip could not be saved to history.")
          }
        } else {
          // Location failed – cannot track
          if (msg.includes("geolocation") || msg.includes("location") || err?.code === 1) {
